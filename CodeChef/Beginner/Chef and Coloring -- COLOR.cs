...
namespace ConsoleApplication1
{
    class Program
    {
        ...
        {
            int repetitions = 0;
            long T = long.Parse(Console.ReadLine()); // reads the number of cases
            if (T > 10) { T = 10; } // Constraint - never read more than 10 cases at a time. 
            int N, red = 0, blue = 0, green = 0; 
            do{
                N = Convert.ToInt32(Console.ReadLine());
                if (N > 100000) { N = 100000; } // Constraint - number of rooms 

                    char[] roomsArray = Console.ReadLine().ToCharArray(); 
                    foreach (char room in roomsArray)
                    { // since each color needs only one change, to turn into one of the others, we accumulate the number of changes needed to turn all the rooms on each color. 
                        switch (room) {
                            case 'R': green++; blue ++; break;
                            case 'G': blue++; red ++; break;
                            case 'B': red++; green ++; break;
                            default: break;
                        }
                    }
                repetitions++;
                Console.WriteLine(Math.Min(green, Math.Min(blue, red))); // the program needs to know the minimum number of rooms he has to repaint, int, so we just show the minium number ignoring the required color. 
                red = blue = green = 0;
            }
            while(repetitions < T);
        }
