...
namespace ConsoleApplication1
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int N = Convert.ToInt16(NK[0]); // number of tweets
            // all the tweets start closed 
            int K = Convert.ToInt16(NK[1]); // number of clicks done
            string[] instructions = new string[K];
            for(int k = 0; k < K; k++)
            { instructions[k] = Console.ReadLine(); }
            bool[] tweets = new bool[N];

            foreach(string instruction in instructions)
            {
                tweetsToggle(ref tweets, instruction.Split());
                Console.WriteLine(tweets.Where(active => active == true).Count());
            }
        }

        static void tweetsToggle(ref bool[] tweet, string[] instruction)
        {
            if (instruction[0] == "CLICK")
            {
                int position = (Convert.ToInt16(instruction[1]) - 1 );
                tweet[position] = !tweet[position];
            }
            else
            {
                for (int k = 0; k < tweet.Count(); k++)
                { tweet[k] = false; }
            }
        }
    }
