...
constraint "fk_est" foreign key(idEstado) references estados(idEstado)
) ;
create table [dbo].[canceladas]
(
	idReposicion int not null,
	idSolicitante varchar(20),
	horaCreacion datetime,
	horaCancelacion datetime,
	idHerramienta int,
	idPuesto int
) ;
...
INSERT INTO [dbo].[sucursales] ([sucursal])  values ('Central'),('Central uno'),('Central dos')
...
insert into [dbo].[herramientas]([herramienta],[stock],[estimado]) values ('Auricular',300,6),('Microfono',300,6),('CPU',300,30),('Monitor',300,20),('Silla',300,15),('Teclado',300,7),('Mouse',300,7)

insert into [dbo].[usuarios] ([username],[claveacc],[idRol]) values ('admin','admin','adm')
...
select idOperario, max(horaResolucion - horaAsignacion),AVG(horaResolucion - horaAsignacion)
from reposiciones r, usuarios u 
where u.idUsuario = r.idOperario and u.idSupervisor = 14 and horaCreacion > '09-01-2014' and horaCreacion < '10-30-2014'
group by idOperario 
order by 2 desc,idOperario
